...
function getDataFromSelectedRange(tableName) {
  var selected = tableContent[tableName].getSelectedLast(); // Get the selected range coordinates [startRow, startCol, endRow, endCol]
  var selectedData = [];

  for (var row = selected[0]; row <= selected[2]; row++) {
    var rowData = [];

    for (var col = selected[1]; col <= selected[3]; col++) {
