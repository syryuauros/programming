...
do
#j=$(($i - 1))
sbatch $c_path${d_path[$i]}"batch_code.sh" $c_path${d_path[$i]} ${code_name[$i]} ${index_list[$i]} &
done
sbatch $c_path${d_path[$((l_d - 1))]}"batch_code.sh" $c_path${d_path[$(($l_d - 1))]} ${code_name[$(($l_d - 1))]} ${index_list[$(($l_d - 1))]}
